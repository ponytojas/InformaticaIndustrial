...
uint8_t auxSeg;							 		 			//Variable que controla el número de digito del display 7-segmentos
uint8_t finDisplay = FALSE;		  		 //Variable que termina con el proceso de mostrar números en el display
uint8_t bit = 16;							 			//Variable que contabiliza el número de veces que se enciende led2 y el digito que led 3 comprueba
uint8_t subidaBajada = FALSE; 		 //Variable que controla si sumamos o restamos
																  //
...
static int auxLed3 = 0;				 //Para evitar solapamientos, utilizamos esta variable para saber que número comprobamos en led3 del buffer
...
int addElement(TIPO data){  //Añadir número. Igual que extraer, pero añadiendo al array del buffer
	if (!isFull()){   
...
void display (uint8_t digito){
	...
	//Realizamos retardos y apagamos el display, para que parpadee y se pueda ver sin problemas el cambio de número, incluso si el digito fuese igual
	osDelay(300);
		...
		if (datoCalculo %2 == 0 && datoCalculo != 2){ //Si el número es par, sumamos 1 para convertirlo en impar antes de emepezar a calcular
			...
			for(i=0;i<64;i++){  
				...
				switch (primo){
					...
					primo = FALSE;
					a = Buffer[auxLed3]; //Obtenemos el valor a mostrar con led3 con su variable auxiliar para evitar solapamientos
					auxLed3++;
					if(auxLed3 == TAM_FIFO){
						auxLed3 = 0;
					}
				...
				switch (subidaBajada){
						case 0: //subir
							if(datoCalculo == 1 || datoCalculo == 2){datoCalculo--;}
							if(datoCalculo+2 >= datoFinal){datoCalculo = datoInicial;} //Si nos pasamos del limite empezamos la cuenta de nuevo por el valor inicial
							/*Utilizamos el datoInicial, y no cogemos el valor de los switches, porque pueden haber variado o puede haberse activado la interrupción 
							externa que modifica el valor inicial*/
							else{datoCalculo += 2;}
							break;
							
						case 1: //bajar
							if (datoCalculo == 2 || datoCalculo == 3){datoCalculo++;}
							if(datoCalculo-2 <= datoInicial){datoCalculo=datoFinal;} //Igual que en el caso 0, pero con el valor final
							else{datoCalculo -= 2;}
							break;
				}
			}
...
int main(){
	...
	osSignalWait(0x01,osWaitForever); //Equivale a un while(1) ya que nunca mandamos la señal necesaria
}
